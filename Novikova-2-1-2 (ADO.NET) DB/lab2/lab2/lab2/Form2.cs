    ...
    public partial class Form2 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (radioButtonGet.Checked)
                command = "select*from [Salon Krasi].[dbo].[Vidi Poslug] ";
            //if (radioButtonGet.Checked)
            //command = "select*from [Salon Krasi].[dbo].[Vidi Poslug] where ID=" + richTextBoxID.Text;

            if (radioButtonAdd.Checked)
            {
                command = "insert into [Salon Krasi].[dbo].[Vidi Poslug] (Name,TimeNeeded) values ('" + richTextBoxName.Text + "'," + richTextBoxTime.Text + ");";
                MessageBox.Show("Entry added");
            }
            if (radioButtonUpdate.Checked)
            {
                command = "update [Salon Krasi].[dbo].[Vidi Poslug] set Name='" + richTextBoxName.Text + "', TimeNeeded= " + richTextBoxTime.Text + " where ID=" + richTextBoxID.Text;
                MessageBox.Show("Entry updated");
            }
            if (radioButtonDelete.Checked)
            {
                command = "delete [Salon Krasi].[dbo].[Vidi Poslug] where ID = " + richTextBoxID.Text;
        ...
        {
            Regex Sample = new Regex("^[а-яА-ЯёЁaіІїЇ-zA-Z]{1,20}$");
